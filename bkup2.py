...
def calculate_fitness(melody):
    ...
    # 4. Mid-range preference: reward notes near the center of the allowed range
    ...
    # 5. Penalize long monotonic descending or ascending runs (discourage continuously going down)
    desc_run = 0
    asc_run = 0
    for i in range(len(notes) - 1):
        ...
        # Penalize long descending runs more strongly
        if desc_run >= 3:
            # each extra step beyond 2 incurs an increasing penalty
            score -= 2 * (desc_run - 2)
        if asc_run >= 4:
            score -= 1 * (asc_run - 3)
    ...
    # 7. Rhythm Variety: Penalize excessive use of short notes (eighth notes)
    eighth_note_count = sum(1 for n in notes if n.duration == 0.5)
    if len(notes) > 0:
        eighth_ratio = eighth_note_count / len(notes)
        if eighth_ratio > 0.5: # If more than 50% are eighth notes
            score -= (eighth_ratio - 0.5) * 20 # Penalize

    # 8. Start Note: Reward starting on Tonic (C) or Dominant (G) or Mediant (E)
    ...
    # 9. Rhythmic Structure (Motif Repetition)
    # Check if Bar 1 rhythm matches Bar 3 (A-B-A-C structure) or Bar 1 matches Bar 2 (A-A-B-C)
    current_time = 0.0
    ...
    # Bonus for rhythmic repetition
    if bar_rhythms[0] and bar_rhythms[2] and bar_rhythms[0] == bar_rhythms[2]:
        score += 15 # Strong reward for A-?-A-? structure
    elif bar_rhythms[0] and bar_rhythms[1] and bar_rhythms[0] == bar_rhythms[1]:
        score += 10 # Reward for A-A-?-? structure

    # 10. Strong Beat Alignment
    # Reward notes that start on strong beats (0.0, 2.0, 4.0, ...)
    current_time = 0.0
    on_beat_score = 0
    for n in notes:
        # Check if current_time is close to an even number (0, 2, 4...)
